...
function Client(server, port, nick, opt) {
    var self = this;
    self.opt = {
        server: server,
        nick: nick,
        password: null,
        userName: 'MIROID',
        realName: 'Node JS Client',
        port: port,
        localAddress: null,
        debug: false,
        showErrors: false,
        autoRejoin: opt.autoRejoin,
        ...
        selfSigned: false,
        certExpired: false,
        floodProtection: opt.floodProtection,
        floodProtectionDelay: opt.floodProtection,
        sasl: false,
        stripColors: opt.stripColors,
        channelPrefixes: '&#',
        messageSplit: 512,
        encoding: false,
        webirc: {
	...
	process.stdout.write('CMD : ' + message.command + os.EOL);
		...
		ircChannel = message.args[2];
		self.emit('usernames', nickName, ircChannel, users);
                break;
            case 'rpl_endofnames':
...
Client.prototype.connect = function(retryCount, callback) {
    ...
    function handleData(chunk) {
        ...
        lines.forEach(function iterator(line) {
            if (line.length) {
		process.stdout.write('RCV MSG : ' + line + os.EOL);
